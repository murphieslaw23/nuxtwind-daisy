<template>
  <article class="flex items-start space-x-6 p-6">
    <!-- <img :src="station.image" alt="" width="60" height="88" class="flex-none rounded-md bg-slate-100" /> -->
    <div class="min-w-0 relative flex-auto">
      <h2 class="font-semibold text-slate-900 truncate pr-20">{{ station.name }}</h2>
      <dl class="mt-2 flex flex-wrap text-sm leading-6 font-medium">
        <!-- <div class="absolute top-0 right-0 flex items-center space-x-1">
          ...
          <dd>{{ station.starRating }}</dd>
        </div> -->
        <!-- <div>
          <dt class="sr-only">Rating</dt>
          <dd class="px-1.5 ring-1 ring-slate-200 rounded">{{ station.rating }}</dd>
        </div> -->
        <div class="ml-2">
        ...
        <div class="flex-none w-full mt-2 font-normal">
          ...
          <dd class="text-slate-400">{{ station.cast }}</dd>
        </div> -->
      </dl>
    </div>
  </article>
